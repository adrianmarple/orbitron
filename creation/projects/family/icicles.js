...
module.exports = () => {
  ...
  let previousEdge = addLine(v0, 4, -135)
  let previousVertex = previousEdge.otherVertex(v0)
  for (let i = 0; i < MAX; i++) {
    let targetPoint = DOWN.scale(i * (3 + Math.log(i/2+1)/2))
      .add(FORWARD.scale(2 * (MAX - i/2)/MAX).rotate(UP, i))
    ...
    let foldNormal = diff.normalize().add(previousEdge.toVector(previousVertex))
